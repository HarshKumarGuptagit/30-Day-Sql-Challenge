...
with month_cases as (
	select 
		...
		sum(cases_reported) cases_rep
	from covid_cases
	group by month
	order by month asc
),
	total_case as (
	select *,sum(cases_rep) over (order by month) as running_total 
	from month_cases
)
select 
	month,case when month>1 
		then
		round((cases_rep/lag(running_total) over (order by month))*100,1)::varchar
		else
		'-'
		end as percentage_increse
from total_case;
