...
SELECT 
	post_id,
	sum((perc_viewed/100)*EXTRACT(EPOCH FROM (session_endtime - session_starttime))::int) as post_time
FROM post_views pv
JOIN user_sessions us on us.session_id=pv.session_id
group by post_id
having sum((perc_viewed/100)*EXTRACT(EPOCH FROM (session_endtime - session_starttime))::int)>5
